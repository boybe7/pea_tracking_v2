  ...
  <div class="row-fluid">
	<div class="span5">
		<?php
          ...
          $list = array();
          $sql = "SELECT DISTINCT pj_manager_name FROM project";
          $command = Yii::app()->db->createCommand($sql);
          $result = $command->queryAll();
          foreach ($result as $key => $value) {
           
            $list[$value['pj_manager_name']] = $value['pj_manager_name'];
          }

          $sql = "SELECT DISTINCT pj_director_name FROM project WHERE pj_director_name NOT IN (SELECT DISTINCT pj_manager_name FROM project)";
          $command = Yii::app()->db->createCommand($sql);
          $result = $command->queryAll();
          foreach ($result as $key => $value) {
             $list[$value['pj_director_name']] = $value['pj_director_name'];
          }


            echo CHtml::label('ผู้จัดการโครงการ/ผู้อำนวยการโครงการ','manager_name');  
            echo CHtml::dropDownList('manager_name', '', $list,array("class"=>'span12'));
