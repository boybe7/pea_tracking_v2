...
class GuaranteeController extends Controller
{
	...
	public function actionCreate($id)
	{
		...
		{
			$model->attributes=$_POST['Guarantee'];
	...
	{
		$model=new GuaranteeTemp;
		if(isset($_POST['Guarantee']))
		{
			$model->attributes=$_POST['Guarantee'];
			$model->contract_no = $id;		
			$model->pj_id = $_POST['pj_id'];	
			if (Yii::app()->request->isAjaxRequest)
	        {
	...
	{
		$model=GuaranteeTemp::model()->findByPk($id);

		if(isset($_POST['GuaranteeTemp']))
		{
			$model->attributes=$_POST['GuaranteeTemp'];
			//$model->last_update =  (date("Y")).date("-m-d H:i:s");
			if (Yii::app()->request->isAjaxRequest)
	         {
	...
	public function actionDeleteTemp($id)
	{
		...
		{
			// we only allow deletion via POST request
			GuaranteeTemp::model()->findByPk($id)->delete();
	...
	{
		$dataProvider=new CActiveDataProvider('Guarantee');
	...
	{
		$model=new Guarantee('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Guarantee']))
			$model->attributes=$_GET['Guarantee'];
	...
	{
		$model=Guarantee::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='guarantee-form')
		{
